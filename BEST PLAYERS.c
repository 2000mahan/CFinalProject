...
int asc_T(const void *i1,const void *i2)
{
    struct info *a = (struct info *)i1;
	struct info *b = (struct info *)i2;
	if(a->Treasury > b->Treasury)
        return -1;
    else if(a->Treasury < b->Treasury)
        return 1;
    else
      return 0;

}
int asc_C(const void *i1,const void *i2)
{
	struct info *a = (struct info *)i1;
	struct info *b = (struct info *)i2;
    if(a->Court > b->Court)
...
int asc_p(const void *i1,const void *i2)
{
	struct info *a = (struct info *)i1;
	struct info *b = (struct info *)i2;

	//return (a->people > b->people) ? 1 : (a->people == b->people) ? 0 : -1;
...
fread(array, sizeof(struct info), 3, fp);
qsort(array, 3, sizeof(struct info), asc_p);
int i;
printf("BEST PLAYERS\n");
for(i=0;i<10;i++)
    printf("%s\n", array[i].name_user);
}
void records(char *name_emperor, int people, int Court, int Treasury)
{
FILE *fp;
struct info name;
...
strcpy(name.name_user, name_emperor);
if(fp=fopen("records.bin", "r")==NULL)
{
...
struct info victim[100];
fp=fopen("records.bin", "r");
if(fread(victim, sizeof(struct info), 10, fp)==10)
{
initial_list();
}
}
...
if(people==0||Court==0||Treasury==0)
{
    ...
    strcpy(S.empire, name_emperor);
     ...
     fclose(fp);
     printf("Ok! Goodbye.");
     records(name_emperor, people, Court, Treasury);
      exit(0);
     }
     else
     {
...
int create_World(int number)
{
FILE *fp;
int buf[100];
int static counter=1;
char victim[201], victim1[201], victim2[201];
...
printf("[3] Create your own World\n");
printf(">");
scanf("%d", &decision);
int res;
if(decision==2)
