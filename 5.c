...
struct node *removing_node(struct node **current, struct node **list)
{
struct node *current1;
for(current1=*list;current1!=NULL;current1=current1->next)
{

if(current1->next==*current)
{
...
{
    if(choice==0) //Due to runtime error
        break;
    if(current==NULL)
        break;
    counter++;
